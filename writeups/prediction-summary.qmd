...
I used cleaned text extracted from claim R files as the primary input data, focusing on de-tagged and normalized text fields. For binary classification, I trained a bidirectional LSTM neural network using an embedding layer and a sigmoid output to predict whether each claim belonged to class 0 or 1. For multiclass prediction, I used a similar bidirectional LSTM architecture but with a softmax output layer to classify claims into five possible categories. The binary model achieved an estimated accuracy of approximately 0.78, while the multiclass model achieved an estimated accuracy of approximately 0.74 on the held-out test set. These results show that the LSTM-based models captured meaningful patterns in the text and were effective for both prediction tasks.
